...
function ExactCommunication({ raw, globalCenter, stage, exampleMode, setExampleMode, showExample }: { raw: RawTensors; globalCenter: number; stage: number; exampleMode: ExampleMode; setExampleMode: (mode: ExampleMode) => void; showExample: () => void }) {
  ...
  return <div className={styles.communication}>
    ...
    <p className={styles.scopeNote}>{globalCenter === example.input_aligned_coordinate_one_based ? <>Channel {outputChannel + 1} and input-aligned coordinate {example.input_aligned_coordinate_one_based} were selected by the <b>{example.selection_rule}</b> rule. This selected example teaches the stated criterion; it is not a claim of biological importance.</> : <>You moved from the selected {exampleMode} example at coordinate {example.input_aligned_coordinate_one_based} to coordinate {globalCenter}. This live cell was not selected by that rule, so either path may now be zero. Choose “Show selected example” to return.</>}</p>
