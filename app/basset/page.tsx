...
export default function BassetPage() {
  ...
  const k562Rank = artifact.outputs.top_predictions.findIndex(item => item.label === "K562") + 1;
  const topTwelve = artifact.outputs.top_predictions.slice(0, 12);
  const k562Prediction = artifact.outputs.top_predictions.find(item => item.label === "K562");
  const displayedPredictions = k562Prediction && !topTwelve.some(item => item.label === "K562") ? [...topTwelve, k562Prediction] : topTwelve;
  ...
  return <main className={styles.page}>
    ...
    <section id="stem" className={styles.section}>
      ...
      <div className={styles.stemGrid}>
        ...
        <article><div className={styles.cardTitle}><small>ONE OUTPUT CELL</small><h3>Convolution at position {kernelPosition + 1}</h3><span>cross-correlation: no reversal</span></div><div className={styles.productRow}>{selectedWeights.map((value, index) => <i key={index} style={{ background: signedColor(value, Math.max(...selectedWeights.map(Math.abs))) }}>{value.toFixed(2)}</i>)}</div><div className={`${styles.equation} ${styles.equationLong}`}><span><small>sum products</small><b>{dotProduct.toFixed(3)}</b></span><i>+</i><span><small>conv bias</small><b>{filter.raw_bias.toFixed(3)}</b></span><i>=</i><span><small>before batch norm</small><b>{afterRawBias.toFixed(3)}</b></span><i>→</i><span><small>after batch norm</small><b>{afterBatchNorm.toFixed(3)}</b></span><i>→</i><strong><small>ReLU</small>{afterRelu.toFixed(3)}</strong></div></article>
      </div>
    ...
    <section id="mixing" className={styles.section}>
      ...
      <div className={`${styles.equationWide} ${styles.equationLong}`}><span><small>3,300 products summed</small><b>{mixing.sum_products.toFixed(3)}</b></span><i>+</i><span><small>raw bias</small><b>{mixing.raw_bias.toFixed(3)}</b></span><i>=</i><span><small>before batch norm</small><b>{mixing.after_raw_bias.toFixed(3)}</b></span><i>→</i><span><small>after batch norm</small><b>{mixing.after_batch_norm.toFixed(3)}</b></span><i>→</i><strong><small>ReLU</small>{mixing.after_relu.toFixed(3)}</strong></div>
    ...
    <section id="dense" className={styles.section}>
      ...
      <div className={styles.outputs}><div><small>OFFICIAL HOXA TUTORIAL SEQUENCE</small><h3>Highest predicted accessibility probabilities</h3><p>Top 12 predictions plus the narrated K562 reader. These are model outputs for one sequence, not experimental measurements.</p></div><div className={styles.outputBars}>{displayedPredictions.map(item => <div key={item.target_index_zero_based} className={item.label === "K562" ? styles.k562 : ""}><b>{item.label}{item.label === "K562" ? ` · rank ${k562Rank}` : ""}</b><i style={{ width: `${item.probability * 100}%` }} /><span>{item.probability.toFixed(3)}</span></div>)}</div><aside><small>K562 · rank {k562Rank}</small><strong>{artifact.outputs.k562_probability.toFixed(3)}</strong><span>target #{artifact.outputs.k562_index_zero_based + 1}</span></aside></div>
    </section>

    <section className={styles.compare}>
      <SectionHeading number="7" eyebrow="CROSS-MODEL MENTAL MODEL" title="Two valid ways to combine distant sequence features">Neither architecture is “the CNN way.” A base-resolution output cannot simply discard positional resolution without a recovery mechanism; dilation is one solution, while encoder–decoder paths, skips, and attention are others. Basset instead predicts sequence-level labels, so aggressive pooling is compatible with its task.</SectionHeading>
